...
الگوریتم استفاده شده در این پروژه \gls{a:dqn} می‌باشد. مقدار $\gamma$ در این الگوریتم $0.8$ انتخاب شده است. یا توجه به \w{state} و  \w{reward} های تعریف شده در فصل \ref{ch:alg} پس از 200 بار تلاش به \w{optpol} می‌رسد.
اما برای بالا رفتن دقت ، 2500 بار \w{agent} مسیر را طی کرده است. دلیل آن بالا رفتن تعداد \w{episode}ها می‌باشد. با این کار مقدار $\epsilon$ کاهش یافته و احتمال جست و جوی \w{pol} جدید آن کاهش می‌یابد.
...
در کل پس یافتن \w{optpol}، تعداد \w{step}ها به 73 رسید. سرعت \w{agent} حول 21 در حال نوسان است و میانگین بدون وزن \w{reward}ها برابر $1.33821$ می‌باشد.
